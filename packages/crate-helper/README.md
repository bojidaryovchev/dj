...
- **`packages/extension/`** — Chrome MV3 extension. Lists supported tabs, owns
  the UI, and exports browser cookies.
- **`packages/crate-helper/`** — local Python server that actually runs
  `yt-dlp`. Extensions are sandboxed and can't spawn processes, so this bridge
  is unavoidable.
...
Double-click **`packages\crate-helper\start.cmd`**. That's the whole install
step — it takes the machine from nothing to running:
...
`packages/crate-helper/bin/` (~310 MB, one time). The helper prefers `bin/`
over anything on PATH, which pins you to yt-dlp's own patched ffmpeg build
instead of whatever old copy happens to be first on PATH. Update both later
with:
...
2. **Load unpacked** → select `d:\repos\dj\packages\extension`
...
every reload. Because the ID is pinned, moving the folder (as the monorepo
restructure did) keeps the same ID: just point **Load unpacked** at the new
path.
...
`library.json` records where each download landed, so re-scanning your tabs
won't re-download. Rows you already hold come back **unticked** with the
...
## What to do with the tracks

Once they're on disk, `packages/track-intelligence/` analyses them — key,
Camelot, BPM, tonal segments. See that package's README.
